...
export class Filters {
    ...
    OnChange() {
        ...
        // console.log(GetGames(this.state, this.teams, this.stadiums, this.upcoming));
        for (const [key, collection] of Object.entries(groupBy(GetGames(this.state, this.teams, this.stadiums, this.upcoming), game => this.toggle === "round" ? game.round : globalData.stadiums.find(s => s.id === game.stadiumId).venue))) {
                        ...
                        DocumentElement({
                            ...
                            children: [
                                DocumentElement({
                                    ...
                                    children: [DocumentElement({ elementType: "p", id: "stadium-header", innerText: globalData.stadiums.find(s => s.id === game.stadiumId).venue })]
                                }),
                                DocumentElement({
                                    elementType: "button",
                                    id: "ticket-btn",
...
function GameInPast(game) {
    const [day, monthName] = game.date.split(" ");
    const monthIndex = new Date(`${monthName} 1`).getMonth();
    const utcDate = new Date(Date.UTC(2026, monthIndex, day));
    const ausDate = utcDate.toLocaleString("en-US", { timeZone: "Australia/Sydney" })
    const now = new Date(
