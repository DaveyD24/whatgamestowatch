...
export class Filters {
    ...
    AddTeams(teams: string[]) {
        for (const team of teams) {
            this.teams.push(team);
        }
        this.OnChange();
    }
    RemoveTeams(teams:string[]) {
        for (const team of teams) {
            this.teams.splice(this.teams.indexOf(team), 1);
        }
        this.OnChange();
    }
    AddStadiums(stadiums:string[]) {
        for (const stadium of stadiums) {
            this.stadiums.push(stadium);
        }
        this.OnChange();
    }
    RemoveStadiums(stadiums:string[]) {
        for (const stadium of stadiums) {
    ...
    OnChange() {
        ...
        if (stadiumContainer === null) { return; }
        ...
        for (const stadium of GetStadiumsByTeamsAndState(this.state, this.teams)) {
            ...
            stadiumContainer?.appendChild(element);
        }
        ...
        if (roundToggle === null || stadiumToggle === null) { return; }
        ...
        for (const [key, collection] of Object.entries(groupBy(GetGames(this.state, this.teams, this.stadiums, this.upcoming), game => this.toggle === "round" ? game.round : game.venue))) {
            ...
            gamesSection?.appendChild(gameCollection);
        }
...
function GetStadiumsByTeamsAndState(state: string, teams: string[]) {
    const stadiums = [];
...
function GetGames(state: string, teams: string[], stadiums: string[], upcoming: boolean) {
    const games = [];
    for (const stadium of globalData["stadiums"]) {
